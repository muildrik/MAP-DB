...
$(document).ready(() => {
    ...
    // UPDATE SAVED SEARCH
    $('.accordion-menu input').on('click keyup', e => {
        key = true
        if ($(e.currentTarget).attr('type') == 'text') {
            $(e.currentTarget).attr('id', `query_${$(e.currentTarget).val()}`)
        ...
        if (key) {
            ...
            $.post('/mygiza/search/all/update/', { 'id' : $('li.is-active[name]').attr('name'), 'param' : $(e.currentTarget).attr('id') })
            .done(response => {
                if (response.success) {
    ...
    $('.del_search').on('click', e => {
        $.post('/mygiza/search/all/del/', { 'id' : $(e.currentTarget).data('id') })
        .done(response => {
            if (response.success) {
    ...
    $('.run_search').on('click', e => {
        console.log('ok')
        $.get('/mygiza/search/lookup/', { 'token' : $(e.currentTarget).data('id') })
        .done(response => {
        //     console.log(response)
